...
function ServiceCard({ num, icon, title, titleJp, desc }: ServiceCardProps) {
  return (
    <div style={{
      background: 'var(--navy-deep)', padding: '40px 32px', position: 'relative',
      borderRight: '1px solid rgba(201,168,76,0.08)', borderBottom: '1px solid rgba(201,168,76,0.08)',
    }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', textAlign: 'center' as const, padding: '0 16px' }}>
      <div style={{
        ...
        border: '1px solid rgba(201,168,76,0.3)', background: 'var(--navy-mid)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontFamily: "'Cormorant Garamond', serif", fontSize: '20px', fontWeight: 300, color: 'var(--gold)',
        marginBottom: '24px',
      }}>{num}</div>
      <div style={{ fontSize: '13px', fontWeight: 300, letterSpacing: '0.12em', color: 'var(--washi)', marginBottom: '10px' }}>{title}</div>
      <p style={{ fontSize: '11.5px', fontWeight: 300, color: 'var(--washi-faint)', lineHeight: 1.9, letterSpacing: '0.04em', whiteSpace: 'pre-line' as const }}>{desc}</p>
    </div>
  )
}
...
const services: ServiceCardProps[] = [
  {
    num: '01', title: 'Export Support', titleJp: '輸出サポート',
    desc: '日本国内の商品調達から輸出書類・通関業者調整・発送まで一括サポート。法令遵守の透明な輸出手続きを代行します。',
    icon: <svg viewBox="0 0 36 36" fill="none" style={{ width: '100%', height: '100%' }}><path d="M6 10l12-6 12 6v10c0 6-5 10-12 12C11 30 6 26 6 20V10z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" /><path d="M13 18l3 3 7-7" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg>,
  },
  {
    num: '02', title: 'Sourcing', titleJp: '商品調達・買い付け',
    desc: '日本限定品・地域限定品・入手困難品も正規流通ルートで調達。和牛・日本酒・コスメ・ホビーなど幅広く対応。',
    icon: <svg viewBox="0 0 36 36" fill="none" style={{ width: '100%', height: '100%' }}><circle cx="18" cy="18" r="12" stroke="currentColor" strokeWidth="1.2" /><path d="M12 12c2-2 8-3 12 0M11 23c2 3 9 5 14 1" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg>,
  },
  {
    num: '03', title: 'Logistics', titleJp: '物流・配送最適化',
    desc: 'DHL・FedEx・EMS・航空貨物・海上輸送を比較し最適なルートを提案。総着地コスト・配送速度・通関リスクを考慮。',
    icon: <svg viewBox="0 0 36 36" fill="none" style={{ width: '100%', height: '100%' }}><rect x="6" y="10" width="24" height="16" rx="1.5" stroke="currentColor" strokeWidth="1.2" /><path d="M6 15h24M12 21h4M20 21h4" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" /></svg>,
  },
  {
    num: '04', title: 'Compliance', titleJp: '法務・コンプライアンス',
    desc: 'HSコード参考提示・輸出規制確認・インコタームズ対応・仕向地輸入規制の事前確認まで対応します。',
    icon: <svg viewBox="0 0 36 36" fill="none" style={{ width: '100%', height: '100%' }}><path d="M10 28V14l8-8 8 8v14" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" /><rect x="14" y="20" width="8" height="8" stroke="currentColor" strokeWidth="1.2" /></svg>,
  },
  {
    num: '05', title: 'Influencer & SNS', titleJp: 'インフルエンサー支援',
    desc: '海外インフルエンサー・クリエイター向けの専用プラン。DM営業・見積・発送まで一気通貫でサポートします。',
    icon: <svg viewBox="0 0 36 36" fill="none" style={{ width: '100%', height: '100%' }}><path d="M8 26l6-8 4 4 4-6 6 10H8z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" /><circle cx="26" cy="12" r="4" stroke="currentColor" strokeWidth="1.2" /></svg>,
  },
  {
    num: '06', title: 'AI / DX', titleJp: 'AI・業務自動化',
    desc: '見積自動生成・多言語自動返信・AIチャットボット搭載。最新DXツールで問い合わせから発送まで高速対応。',
    icon: <svg viewBox="0 0 36 36" fill="none" style={{ width: '100%', height: '100%' }}><rect x="8" y="8" width="20" height="20" rx="2" stroke="currentColor" strokeWidth="1.2" /><circle cx="18" cy="18" r="3.5" stroke="currentColor" strokeWidth="1.2" /></svg>,
  },
]

const trustItems = [
  '古物商許可証 第305581606050号',
  '税関輸出入者コード 10000NKS0000',
  '初回1ヶ月 手配手数料無料',
  '法令遵守 / Full Compliance',
  'DHL / FedEx / EMS / 航空 / 海上',
]

const aboutStats = [
  { num: '01', title: '法令遵守 / Full Compliance', desc: '日本輸出法令・仕向地輸入規制を厳守した透明な取引' },
  { num: '02', title: '小ロット対応 / MOQ from 1 unit', desc: '1個からの小ロット・サンプル出荷に対応' },
  ...
  return (
    <>
      ...
      <section style={{ padding: 'var(--section-pad) var(--gutter)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(48px,6vw,100px)', alignItems: 'center' }}>
        <ScrollReveal>
          ...
          <p className="section-body">
            YUKIMICHI – SNOWPATH JAPAN は、日本法人 JUSTHEN Co., Ltd. が運営する輸出支援サービスです。<br /><br />
            雪道は険しくとも、一歩ずつ正しく進めば必ず目的地へ辿り着ける——その信念を輸出という仕事に重ねています。<br /><br />
            We source authentic products legally in Japan and deliver them worldwide with full compliance and transparency.
          </p>
      ...
      <section style={{ padding: 'var(--section-pad) var(--gutter)', background: 'linear-gradient(180deg, var(--navy-deep) 0%, var(--navy-mid) 100%)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0, background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.08)' }} className="services-grid">
      ...
      <section style={{ padding: 'var(--section-pad) var(--gutter)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }} className="flow-grid">
